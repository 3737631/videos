...
export default function App() {
  ...
  const handleDownloadYt = async () => {
    ...
    setYtError(""); setYtLoading(true); setStatus("Descargando Shorts sin marca...");
    try {
      clearMemory(); if(finalVideo) { try{URL.revokeObjectURL(finalVideo)}catch{}; setFinalVideo(null); }
      // Reusa fetchTikTokClips que ya maneja YT via /api/yt
      const { clips: ytClips, errors } = await fetchTikTokClips(ytLinks.join("\n"), (m)=>setStatus(m));
      setClips(ytClips);
      setTotalDuration(Math.min(15, Math.max(8, Math.round(ytClips.reduce((s,c)=>s+c.playDuration,0)))));
      if (errors.length) setYtError(`Descargados ${ytClips.length} OK. Errores: ${errors.join(" | ").slice(0,200)}`);
      setStep(2); setStatus("");
