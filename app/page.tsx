  ...
  useEffect(() => {
    const h = async (e: MessageEvent) => {
      ...
      if (e.data?.type === "TIKTOK_LINKS" && Array.isArray(e.data.links)) {
        ...
        if (isYT) {
          try {
            setTiktokLoading(true); setStatus("Preparando 3 vídeos rápidos...");
            const createOne = async (u: string): Promise<VideoClip | null> => {
              ...
              try {
                ...
                const chunks:BlobPart[]=[]; const rec=new MediaRecorder(stream,{mimeType: MediaRecorder.isTypeSupported("video/webm;codecs=vp9")?"video/webm;codecs=vp9":"video/webm"}); rec.ondataavailable=e=>{if(e.data.size>0) chunks.push(e.data)};
                const dur=4;
            ...
            if (ytClips.length>0) {
              ...
              setTiktokError("No se pudieron preparar");
            }
          } catch {}
