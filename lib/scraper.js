...
function detectBlocked(dom, payloads) {
  const text = `${dom.bodyText || ""} ${dom.ogTitle || ""}`.toLowerCase();
  const patterns = [
    /captcha|verify you are human|pardon our interruption|cf-chl|access denied|forbidden|verify\s/i,
    /被.*拦|访问.*验证|验证码|安全验证/i,
  ];
  for (const re of patterns) {
    if (re.test(text)) return "captcha";
  }
  const noItem =
    /esta p\w+ no existe|page does not exist|p\w+ no disponible|article not available|handled with care/i.test(
      text
    );
  const hasData = payloads.length > 0 || dom.ogTitle || (dom.dida && dom.dida.itemData);
  if (noItem && !hasData) return "no-disponible";
  return "";
}

function extractManufacturer(attributes) {
  const at = {};
  ...
  const pick = (keys) => {
    for (const k of keys) {
      if (at[k] && at[k].trim()) return at[k].trim();
    }
    return "";
  };

  const fabricante = pick([
    "fabricante",
    "manufacturer",
    "fabricante del producto",
    "fabricante original",
  ]);
  const marca = pick(["marca", "brand", "nombre de la marca", "marca registrada"]);
  const modelo = pick([
    "modelo",
    "model",
    "nombre del modelo",
  ...
  for (const a of attributes || []) {
    const m = String(a.value || "").match(emailRe);
    if (m) {
      email = m[0];
      break;
    }
  ...
  let confianza = "no-verificado";
  if (fabricante && (direccion || email)) confianza = "alta";
  else if (fabricante) confianza = "media";
  else if (marca) confianza = "baja";

  return {
    fabricante,
    fabricante_email: email,
    fabricante_direccion: direccion,
    fabricante_pais: pais,
    marca,
    modelo,
    confianza,
  };
...
function buildResult(url, payloads, dom) {
  ...
  const manufacturer = extractManufacturer(attributes);
  const blocked = detectBlocked(dom, payloads);

  if (!title && !images.length && !attributes.length) {
    if (blocked === "captcha") {
      throw new Error(
        "AliExpress ha bloqueado la solicitud (CAPTCHA o verificación). Prueba de nuevo o usa el enlace móvil del producto."
      );
    }
    if (blocked === "no-disponible") {
      throw new Error(
        "El producto no está disponible o la página no existe. Verifica el enlace."
      );
    }
    throw new Error(
  ...
  return {
    ...
    modelo: manufacturer.modelo,
    fabricante: manufacturer.fabricante,
    fabricante_email: manufacturer.fabricante_email,
    fabricante_direccion: manufacturer.fabricante_direccion,
    fabricante_pais: manufacturer.fabricante_pais,
    confianza: manufacturer.confianza,
    blocked,
