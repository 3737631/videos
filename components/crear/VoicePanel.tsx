...
export function VoicePanel({
  ...
  const previewScript = async () => {
    ...
    try {
      ...
      const r = await synthesizeProsody(firstLine, voiceId, { styleId: styleId ?? undefined });
      playBlob(r.blob);
    } catch (e) {
    ...
    <section className="cc-card p-5">
      ...
      >
        {VOICE_CATALOG.map((v) => (
          <option key={v.id} value={v.id}>
            {v.name} · {v.locale}
            {installedIds.includes(v.id) ? " · lista" : ""}
          </option>
        ))}
      </select>
      ...
      {preparingVoice && (
        <div className="mt-2 rounded-xl border border-violet-400/30 bg-violet-500/10 px-3 py-2 text-xs text-violet-200">
          Preparando voz… {preparePct != null ? `${preparePct}%` : "descargando"}
          {preparePct != null && (
      ...
      {!preparingVoice && installedIds.includes(voiceId) && (
        <div className="mt-2 text-xs font-medium text-emerald-300">Voz lista</div>
      )}
